            ...
            <section className="flex flex-col md:flex-row md:items-end justify-between gap-6">
                <div className="space-y-1">
                    <div className="flex items-center gap-2 text-primary font-bold tracking-widest text-xs uppercase">
                        <Link href="/products" className="hover:underline">Inventory</Link> / Categories
                    </div>
                    <h1 className="text-4xl font-black tracking-tight">Product Categories</h1>
                    <p className="text-muted-foreground">Classify your products for better organization and reporting.</p>
                </div>
            </section>

            <div className="grid grid-cols-1 lg:grid-cols-12 gap-8 items-start">

                {/* Form Section - Side Grid Form */}
                <div className="lg:col-span-4 space-y-6">
                            ...
                            <div className="relative z-10">
                                <div className="flex items-center gap-3 mb-6">
                                    ...
                                    <h3 className="text-xl font-black tracking-tight">
                                        {editingId ? "Update Category" : "Quick Add"}
                                    </h3>
                                </div>

                                <form onSubmit={handleSubmit} className="space-y-4">
                                    <div className="space-y-2">
                                        <label className="text-[10px] font-black uppercase tracking-widest text-muted-foreground ml-1">Category Code</label>
                                        <input
                                            required
                                            type="text"
                                            placeholder="e.g. ELEC, OFFICE"
                                            value={formData.code}
                                    ...
                                    <div className="space-y-2">
                                        ...
                                        <textarea
                                            ...
                                            className="w-full bg-muted/50 border rounded-xl p-4 font-bold outline-none focus:border-primary/50 transition-all resize-none"
                                        />
                                    </div>
                                    ...
                                    <div className="pt-4 flex gap-2">
                                        ...
                                        >
                                            {isSaving ? <Loader2 size={16} className="animate-spin" /> : (editingId ? "Save Changes" : "Register Category")}
                                        </button>
                                    </div>
                                </form>
                            </div>
                    ...
                    <div className="p-6 bg-muted/30 border border-dashed rounded-[2rem] text-center space-y-2">
                        <Tag size={24} className="mx-auto text-muted-foreground opacity-50" />
                        <p className="text-[10px] font-black text-muted-foreground uppercase tracking-tighter">Hierarchy Tip</p>
                        <p className="text-xs text-muted-foreground/80 leading-relaxed font-semibold">Categories help in generating advanced inventory valuation reports and tax filings.</p>
                    </div>
                </div>

                {/* List Section - Grid of Categories */}
                        ...
                        <AnimatePresence>
                            {loading ? (
                                ...
                                <div className="col-span-full h-80 flex flex-col items-center justify-center bg-card border border-dashed rounded-[2.5rem] text-muted-foreground space-y-4 font-bold">
                                    ...
                                    <p className="text-xl">Zero categories found.</p>
                                </div>
                            ) : (
                                filteredCategories.map((cat, i) => (
                                    <motion.div
                                        ...
                                        className={cn(
                                            ...
                                            editingId === cat.id ? "border-primary shadow-xl ring-1 ring-primary/20" : "hover:shadow-xl hover:border-primary/10"
                                        )}
                                    >
                                        <div className="flex items-start justify-between mb-4">
                                            <div className="flex items-center gap-4">
                                                ...
                                                <div>
                                                    <h4 className="text-lg font-black tracking-tight">{cat.name}</h4>
                                                    <p className="text-[10px] font-black uppercase text-primary/60 tracking-widest">{cat.code}</p>
                                                </div>
                                            </div>
